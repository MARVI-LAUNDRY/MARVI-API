...
export async function getServicesService({page, limit, search, sortBy = 'nombre', sortOrder = 'asc'}) {
    const allowedSortFields = new Set(['codigo', 'nombre', 'precio', 'createdAt']);
    const safeSortBy = allowedSortFields.has(sortBy) ? sortBy : 'nombre';
    const safeSortOrder = sortOrder === 'desc' ? -1 : 1;
    const cacheKey = `services:list:p${page}:l${limit}:q${search}:s${safeSortBy}:o${safeSortOrder}`;
    ...
    if (['codigo', 'nombre'].includes(safeSortBy)) {
        servicesQuery.collation({locale: 'es', strength: 1});
    }

    const [total, services] = await Promise.all([Service.countDocuments(filter), servicesQuery.lean()]);
